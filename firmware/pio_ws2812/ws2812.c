...
void pattern_snakes(PIO pio, uint sm, uint len, uint t) {
    for (uint i = 0; i < len; ++i) {
        uint x = (i + (t >> 1)) % 64;
        if (x < 10)
            put_pixel(pio, sm, urgb_u32(100, 0, 0));
        else if (x >= 15 && x < 25)
            put_pixel(pio, sm, urgb_u32(0, 100, 0));
        else if (x >= 30 && x < 40)
            put_pixel(pio, sm, urgb_u32(0, 0, 100));
        else
            put_pixel(pio, sm, 0);
    }
}
...
void pattern_sparkle(PIO pio, uint sm, uint len, uint t) {
    ...
    for (uint i = 0; i < len; ++i)
        ...
        put_pixel(pio, sm, rand() % 16 ? 0 : urgb_u32(100, 100, 100));
        // put_pixel(pio, sm, rand() % 16 ? 0 : 0x64);
}
